...
try_command scp -i ~/.ssh/id_ed25519 -r ./frontend/* karthikeya@35.230.16.65:/tmp
...
try_command ssh -i ~/.ssh/id_ed25519 karthikeya@35.230.16.65 "sudo rm -rf /var/www/html/'$backend_tech' && sudo mkdir -p /var/www/html/'$backend_tech' && sudo mv -f /tmp/* /var/www/html/'$backend_tech'"
...
rsync -av --exclude-from='deploy-ignore.txt' -e "ssh -i ~/.ssh/id_ed25519" ./backend/ karthikeya@35.230.16.65:$dir_name
...
# try_command scp -i ~/.ssh/id_ed25519 -r ./backend/* karthikeya@35.230.16.65:$dir_name
# echo -e "\e[32mCopied backend files to project directory\e[0m"


#Copy nginx config to temporary directory
try_command scp -i ~/.ssh/id_ed25519 ./nginx.config karthikeya@35.230.16.65:/tmp
...
try_command ssh -i ~/.ssh/id_ed25519 karthikeya@35.230.16.65 "sudo mv -f /tmp/nginx.config /etc/nginx/sites-available/default"
...
# try_command ssh -i ~/.ssh/id_ed25519 karthikeya@35.230.16.65 "sh docker-clear.sh"
# echo -e "\e[32mDocker Cleared\e[0m"

#Run Docker compose
try_command ssh -i ~/.ssh/id_ed25519 karthikeya@35.230.16.65 "cd '$dir_name' && docker compose -f docker-compose.prod.yml up -d"
echo -e "\e[32mDocker Images, Containers and Volumes created successfully\e[0m"

#Restart nginx
try_command ssh -i ~/.ssh/id_ed25519 karthikeya@35.230.16.65 "sudo systemctl restart nginx"
